    ...
    def __init__(self):
        if not self.initialized:
            ...
            if self.size == 4:
                ...
                line_1 = [200, -25, 8]+[6]*((self.size-6)/2); line_1 += line_1[::-1]
                line_2 = [-25, -50, -4]+[-4]*((self.size-6)/2); line_2 += line_2[::-1]
...
class Player_Reversi(Game_Reversi):
    ...
    def evaluation(self, ennemy, board, depth):
        ...
        # Mobility
        player_m, ennemy_m = self.can_play(board), ennemy.can_play(board)
        player_mobi, ennemy_mobi = len(player_m), len(ennemy_m)
