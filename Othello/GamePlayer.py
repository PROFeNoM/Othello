...
Noms : 
    ...
    WESCHLER Noé
Description :
    Contient ce que dont GameEngine à besoin pour savoir si un
    joueur (Humain ou non) peut jouer et où si tel est le cas.
    Permet de plus de modifier le plateau de jeu en conséquence du
    coup d'un joueur.
"""
...
class PlayerEngine(object):
    ...
    def play(self, column, row, player, swap=False, board=None):
        """
        ...
        :param board: Plateau utilisé au cours de l'évalutation pour
        l'algorithme AlphaBeta/Minimax.
        """
        if self.board.spot(column, row, board) is not ".": # Spot is already filled
            return False
        for dc, dr in self.c.DIRECTIONS:
            ...
            while self.board.is_on_board(c, r) and self.board.spot(self.c.NUM_COL[c], str(r), board) == self.c.ENNEMY_DISK[player.disk]:
                c += dc
                r += dr
                amount = True
            if not self.board.is_on_board(c, r) or amount is None or not self.board.spot(self.c.NUM_COL[c], str(r), board) == player.disk:
                continue 
            ...
            if swap: # We change the board
                ...
                while (c_swap, r_swap) != (c,r):
                    self.board.place(self.c.NUM_COL[c_swap], str(r_swap), player.disk, board)
                    c_swap += dc
                    r_swap += dr
            else:
    ...
    def get_moves(self, player, board=None):
        ...
        l'algorithme AlphaBeta/Minimax.
        """
        return [c+str(r) for c in self.c.COL for r in self.c.RW if self.play(c, str(r), player, False, board)]
