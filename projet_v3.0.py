...
class Board_Reversi(object):
    ...
    def __init__(self, size=10):
        ...
        if not self.initialized:
            print "BONJOUR"
            Board_Reversi.board = {"d" : size, "grille": ['.']*(size**2)}
            Board_Reversi.POS = {column + str(row) : x+size*y for x, column in enumerate([chr(i) for i in range(ord("A"), ord("A")+27)][:size]) for y, row in enumerate(range(1,size+1))}
            Board_Reversi.POS.update({c : i for i,c in enumerate([chr(i) for i in range(ord("A"), ord("A")+27)][:size])}) # Colonne str -> int
            Board_Reversi.POS.update({i : c for i,c in enumerate([chr(i) for i in range(ord("A"), ord("A")+27)][:size])}) # Colonne int -> str
            ...
            for pos, disk in zip([Board_Reversi.POS[key] for key in (Board_Reversi.POS[size/2 - 1] + str(size/2), Board_Reversi.POS[size/2] + str(size/2 + 1), Board_Reversi.POS[size/2 - 1] + str(size/2 + 1), Board_Reversi.POS[size/2] + str(size/2))], "OOXX"): # Positions initiales
